...
You'll also get a little more practice with data filtering and tidying and meet the **`GGally`** package and some penguins. `r emo::ji("penguin")` `r emo::ji("penguin")`
...
We will first use PCA on a dataset with relatively few dimensions. The data are available in the  **`palmerpenguins`** `r Cite(myBib, "palmerpenguins")` package which is based on data in `r Cite(myBib, "Gorman2014")`
...
We have rows with missing values which we will exclude from our analysis.
...
Dealing with missing data is a much discussed are in ML. Several data imputation methods exist
...
---
#  Explore Penguins
```{r ref.label = 'peng-pairs', echo = FALSE, out.width="70%"} 
```
...
percent3 <- res["Cumulative Proportion","PC3"] * 100

```

--

`r percent1`% of the variation is captured in the first PC; `r percent2`% in the first 2 together; and `r percent3`% in the first 2 together
...
```{r ref.label = 'rna-tsne', echo = FALSE, out.width="700px", out.height="550px"} 
```
...
* Identifying cell populations with scRNASeq `r Cite(myBib, "Andrews2018-cy")` A good overview of different experimental protocols for Single-cell RNASeq and the most popular methods for facilitating the computational analysis. 
