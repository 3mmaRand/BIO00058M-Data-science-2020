...
One problem with this approach is that we cannot be sure model we have fitted to our data is generalisable. That is, we do not know how well the model would predict the responses for a new data set.  This is known as *overfitting*.
...
A key process in using supervised ML methods is to build - or train - the model on approximately 75% of the data and then test it on the remaining 25%. 
...
.footnote[
.font50[
We often use the word "class" rather than "group" in ML
]
]
---
# Outline
...
If you load **`MASS`** after **`dplyr`** (or **`tidyverse`**) `MASS::select()` will be used rather than `dplyr::select()` 
