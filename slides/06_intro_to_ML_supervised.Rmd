...
`r emo::ji("loudspeaker")` Do not load the package!
...
`r emo::ji("clapper")` Select the four variables and pipe into the `MASS::lda()` function which does the LDA:
...
`r emo::ji("clapper")` Select the model variables from `Penguin` and predict the species from the `lda` model object:
...
accuracy <- round(res$overall["Accuracy"]*100, 2)
...
-  there were `r adelie_num` Adelie Penguins of which `r adelie_wrong` were predicted incorrectly.  
-  `r chin_num` Chinstrap Penguins of which `r chin_wrong` were predicted incorrectly.  
-  and `r gentoo_num` Gentoo Penguins of which `r gentoo_wrong` were predicted incorrectly.
...
`r emo::ji("clapper")` And predict classes of the **test data**:
...
The model had an accuracy of `r accuracy`%. You may have greater or fewer because the training and testing sets were random selections.
