...
`r emo::ji("clapper")` Have a look at my version of [chaffinches](https://github.com/3mmaRand/BIO00058M-chaffinches-w01)
...
`r emo::ji("clapper")` Make sure you understand why we have used `read_table()` rather than `read_table2()` or `read_csv()` (use `?read_table`)
...
`r emo::ji("clapper")` You may wish to close the RStudio Project you are currently working in and open the one used while coding along with the slides. Alternatively, continue in this project and copy in the code used to:  
...
`r emo::ji("clapper")` Extract the top protein identifier from the `accession` column and put it in a column called `protid`. The top protein identifier is the first Uniprot ID after the "1::" in the `accession` column. 
