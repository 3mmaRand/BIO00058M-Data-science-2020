...
# I have used janitor::clean_names() to ensure the column names are 
# lowercase without spaces and that the year columns have an x 
# in front of them. both these mean I will not need to use backticks 
# around column names such as hdi$`HDI Rank (2018)` and hdi$`1990`  
```

```{r task-1-tidy}
# the format of the data would be improved by having a single 
# observation (i.e., index) per row with other columns giving the country, 
# year and 2018 rank.
